...
const Player = ({ audio, progress, playerRef, setSelectedSong, audios }: SongPlayer) => {
    ...
    function nextSong(forward: boolean) {
        ...
        if (audios && audios[index].id && audios[index].id > 0)
    ...
    return <div className="player">
        {
            playerRef.current && <>
                ...
                <div className="details">
                    <img src={audio ? audio.songImage : ''} />
                    <div>
                        <span>{audio ? audio.songName : ''}</span>
                        <span>{audio ? audio.author : ''}</span>
                    </div>
                </div>
                <div className="bar">
                    <TimeFormat time={progress.currentTime} />
                    {
                        ...
                        }} disabled={audio.cloudinaryId.length === 0} style={{
                            background: linear
                        }} />
                    }
